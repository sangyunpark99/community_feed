...
import org.sangyunpark99.auth.repository.jpa.JpaEmailVerificationRepository;
...
public class EmailVerificationRepositoryImpl implements EmailVerificationRepository {
    ...
    public void verifyEmail(Email email, String token) {
        ...
        EmailVerificationEntity entity =
                jpaEmailVerificationRepository.findByEmail(emailAddress).orElseThrow(() -> new IllegalArgumentException(
                        "인증을 요청하지 않은 이메일 입니다."));
    ...
    public boolean isEmailVerified(Email email) {
        EmailVerificationEntity entity =
                jpaEmailVerificationRepository.findByEmail(email.getEmailText()).orElseThrow(() -> new IllegalArgumentException(
                "인증을 요청하지 않은 이메일 입니다."));
